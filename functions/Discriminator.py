...
negative_words = ["인공지능", "AI", "가족", "감형", "딸", "아들", "자녀", "자식", "선처", "모범수"]
...
for comment in negative_list :
    ...
    if len(find) != 0 :
        ...
        # 실제 형량보다 낮은 값을 제시했을 경우 반어법일 확륭이 높음. 넘기기
        if max <= preset :
            continue
        
        # 두 개의 차트가 만들어질 예정, 하나는 처벌 차트, 하나는 징역 수위 차트. 그래서 징역 수위 차트는 여기서 미리 정렬
        # 값을 하나하나 다 짚다보면 너무 값이 많아짐. 10년 단위로 끊어서 저장하기
